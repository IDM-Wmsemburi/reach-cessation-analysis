...
<<robustness-display, results='asis'>>=
if (!is.null(robustness_results)) {
  # needs: library(dplyr); library(stringr)
  rob_table <- robustness_results %>%
    dplyr::mutate(
      cv_display = sprintf("%.3f", cv),
      range_display = sprintf("%.1f", range_width),
      robust_display = dplyr::case_when(
        robustness == "Very Robust" ~ "\\textcolor{ForestGreen}{Very Robust}",
        robustness == "Robust" ~ "\\textcolor{blue}{Robust}",
        robustness == "Moderately Robust" ~ "\\textcolor{orange}{Moderate}",
        TRUE ~ "\\textcolor{red}{Not Robust}"
      )
    )
  ...
  cat("\\toprule\n")
  cat("\\textbf{Model} & \\textbf{Method} & \\textbf{CV} & \\textbf{Range} & \\textbf{Assessment} \\\\\n")
  cat("\\midrule\n")
  ...
  # Interpretation
  mean_cv_u5mr <- mean(rob_table$cv[rob_table$model == "U5MR"], na.rm = TRUE)
  mean_cv_imr  <- mean(rob_table$cv[rob_table$model == "IMR"],  na.rm = TRUE)

  cat("\\vspace{0.3cm}\n")
  cat("\\textbf{Interpretation:} Coefficient of variation (CV) measures relative dispersion across scenarios.\n")
  cat("\\begin{itemize}\n")
  cat(sprintf("\\item U5MR estimates: Average CV = %.3f\n", mean_cv_u5mr))
  cat(sprintf("\\item IMR estimates: Average CV = %.3f\n",  mean_cv_imr))
  cat("\\item CV $< 0.2$ indicates robust estimates\n")
  cat("\\end{itemize}\n")

} else {
  cat("\\textbf{Robustness metrics:}\n")
  cat("\\begin{itemize}\n")
  cat("\\item Coefficient of variation across scenarios\n")
  cat("\\item Range width assessment\n")
  cat("\\item Model consistency evaluation\n")
  cat("\\end{itemize}\n")
}
@
\end{frame}
...
\begin{columns}
\begin{column}{0.5\textwidth}
<<method-comparison, results='asis'>>=
if (!is.null(all_results)) {
# 1) Aggregate across scenarios first (mean of medians), then widen
  comparison <- all_results %>%
    dplyr::filter(!is.na(median)) %>%
    dplyr::group_by(model, type) %>%
    dplyr::summarise(median = mean(median, na.rm = TRUE), .groups = "drop") %>%
    tidyr::pivot_wider(names_from = type, values_from = median) %>%
    dplyr::mutate(
      # guard against divide-by-zero
      analytical = ifelse(is.na(analytical) | analytical == 0, NA_real_, analytical),
      diff = crossing - analytical,
      pct_diff = 100 * diff / analytical
    )

  if (nrow(comparison) > 0) {
    u5mr_diff <- comparison$pct_diff[comparison$model == "U5MR"]
    imr_diff  <- comparison$pct_diff[comparison$model == "IMR"]

    cat("\\textbf{Analytical vs Crossing:}\n")
    cat("\\begin{itemize}\n")
    if (length(u5mr_diff) > 0 && all(!is.na(u5mr_diff))) {
      cat(sprintf("\\item U5MR: Crossing %.1f\\%% higher on average\n",
                  mean(u5mr_diff, na.rm = TRUE)))
    }
    if (length(imr_diff) > 0 && all(!is.na(imr_diff))) {
      cat(sprintf("\\item IMR: Crossing %.1f\\%% higher on average\n",
                  mean(imr_diff, na.rm = TRUE)))
    }
    cat("\\end{itemize}\n")
  }
} else {
  cat("\\textbf{Method comparison:}\n")
  cat("\\begin{itemize}\n")
  cat("\\item Analytical: Direct formula solution\n")
  cat("\\item Crossing: Numerical intersection\n")
  cat("\\item Generally consistent results\n")
  cat("\\end{itemize}\n")
}
...
<<final-sensitivity-summary, results='asis'>>=
if(!is.null(all_results) && !is.null(robustness_results)) {
  # Get overall ranges
  u5mr_range <- all_results %>% 
    filter(model == "U5MR", !is.na(median)) %>% 
    summarise(min_med = min(median), max_med = max(median))
  
  imr_range <- all_results %>% 
    filter(model == "IMR", !is.na(median)) %>% 
    summarise(min_med = min(median), max_med = max(median))
  
  # Most robust estimates
  most_robust <- robustness_results %>% 
    arrange(cv) %>% 
    slice_head(n = 2)
  
  if(nrow(u5mr_range) > 0 && nrow(imr_range) > 0) {
    cat(sprintf("\\textbf{Summary of sensitivity analysis:}"))
    cat("\\begin{itemize}")
    cat(sprintf("\\item U5MR thresholds range: %.1f - %.1f per 1,000", 
                u5mr_range$min_med, u5mr_range$max_med))
    cat(sprintf("\\item IMR thresholds range: %.1f - %.1f per 1,000", 
                imr_range$min_med, imr_range$max_med))
    cat("\\item Estimates show good consistency across scenarios")
    cat("\\item Method differences are within uncertainty bounds")
    cat("\\end{itemize}")
  }
  
  if(nrow(most_robust) > 0) {
    cat("\\vspace{0.3cm}")
    cat("\\textbf{Most robust estimates:}")
    cat("\\begin{enumerate}")
    for(i in 1:nrow(most_robust)) {
      row <- most_robust[i,]
      cat(sprintf("\\item %s %s method (CV = %.3f)", 
                  row$model, str_to_title(row$type), row$cv))
    }
    cat("\\end{enumerate}")
  }
  ...
  cat("\\begin{itemize}")
  cat("\\item Consistent thresholds across modeling approaches")
  cat("\\item Limited sensitivity to site exclusions")
  cat("\\item Both analytical and crossing methods converge")
  cat("\\item Results support evidence-based cessation criteria")
  cat("\\end{itemize}")
}
@
