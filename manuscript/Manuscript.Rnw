...
u5 <- thr %>% dplyr::filter(tolower(model) %in% c("u5mr","u5","u5m")) %>%
  dplyr::transmute(txt = sprintf("%.1f (%.1f-%.1f)", threshold_med_1000, threshold_lo_1000, threshold_hi_1000)) %>% 
  dplyr::pull(txt) %>% .[1]
...
\item \textbf{Cessation thresholds estimated:} U5MR \emph{median (95\% CrI)} = \Sexpr{u5} per 1,000; IMR \emph{median (95\% CrI)} = \Sexpr{imr} per 1,000.
